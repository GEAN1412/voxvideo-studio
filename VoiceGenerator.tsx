...
export const VoiceGenerator: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {VOICES.map((voice) => (
          ...
          >
            ...
            <div onClick={(e) => { e.stopPropagation(); handleGenerate("Halo", voice.id, true); }} className="p-2 bg-slate-700 rounded-lg text-slate-300">
      ...
      {audioUrl && <button onClick={() => { const l=document.createElement('a'); l.href=audioUrl; l.download='voice.wav'; l.click(); }} className="w-full py-3 bg-slate-800 border border-slate-700 rounded-xl text-white flex items-center justify-center space-x-2"><span>Unduh (.wav)</span></button>}
      {error && <div className="p-3 bg-red-900/20 border border-red-500/50 text-red-400 rounded-lg text-sm">{error}</div>}
      <button onClick={() => handleGenerate(text, selectedVoice)} disabled={isGenerating || !text.trim()} className="w-full py-4 bg-blue-600 rounded-xl font-bold text-white shadow-lg disabled:opacity-50 transition-all">{isGenerating ? 'Memproses...' : 'Generate Suara'}</button>
    </div>
  );
};
